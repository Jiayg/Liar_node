...
import { ExecutionContext, UnauthorizedException, Injectable, Logger } from '@nestjs/common'
import { TokenService, UserService } from '../user/services'
...
export class JwtAuthGuard extends AuthGuard('jwt') {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      const isValidAccessToken = await this.tokenService.verifyToken(accessToken.replace('Bearer ', ''))
      ...
      const isValidRefreshToken = await this.tokenService.verifyToken(refreshToken)
      ...
      if (user) {
        const tokens = await this.tokenService.refreshToken(isValidRefreshToken)
